...
const CoursesInProgress = () => {
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      ...
      try {
        const response = await fetch(`http://localhost:5000/pensum/${userId}`);
        ...
        if (response.ok) {
          ...
          const coursesWithGrades = await Promise.all(inProgressCourses.map(async (course) => {
            const assignmentsResponse = await fetch(`http://localhost:5000/assignments/${course.id}?userId=${userId}`);
