...
const Pensum = ({ semesters, setSemesters }) => {
  ...
  const handleRemove = async () => {
    if (contextMenu) {
      ...
      await apiCall('http://35.170.26.81:5000/remove-course', 'POST', {
  ...
  const handleStatusUpdate = async (status) => {
    if (contextMenu) {
      ...
      await apiCall('http://35.170.26.81:5000/update-course-status', 'POST', {
        userId, // Include userId in the payload
        semesterName: semesters[contextMenu.semesterIndex].name,
        courseName: contextMenu.course.name,
        status,
  ...
  const addCourse = async (semesterIndex, courseName) => {
    try {
      ...
      const response = await apiCall('http://35.170.26.81:5000/add-course', 'POST', {
        userId, // Include userId in the payload
        semesterName,
        courseName,
      });
  ...
  const addSemester = async () => {
    ...
    try {
      const response = await apiCall('http://35.170.26.81:5000/add-semester', 'POST', {
  ...
  const removeSemester = async (semesterIndex) => {
    ...
    try {
      await apiCall('http://35.170.26.81:5000/remove-semester', 'POST', {
        userId: localStorage.getItem('userId'),
