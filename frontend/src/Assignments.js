...
const Assignments = () => {
  ...
  useEffect(() => {
    const fetchAssignments = async () => {
      ...
      const response = await fetch(`http://35.170.26.81:5000/assignments/${courseId}?userId=${userId}`);
  ...
  const handleAddAssignment = async () => {
    const userId = localStorage.getItem('userId');
    const response = await fetch('http://35.170.26.81:5000/add-assignment', {
      method: 'POST',
  ...
  const handleUpdateAssignment = async (id, field, value) => {
    ...
    const response = await fetch(`http://35.170.26.81:5000/update-assignment/${id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
